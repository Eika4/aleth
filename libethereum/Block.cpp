...
//		ex << errinfo_vmtrace(vmTrace(_block.block, _bc, ImportRequirements::None));
    ...
    DEV_TIMED_ABOVE("applyRewards", 500)
        applyRewards(rewarded, _bc.sealEngine()->blockReward(m_currentBlock.number(), m_currentBlock.difficulty()));
...
void Block::commitToSeal(BlockChain const& _bc, bytes const& _extraData)
{
    ...
    assert(_bc.sealEngine());
    applyRewards(uncleBlockHeaders, _bc.sealEngine()->blockReward(m_currentBlock.number(), m_currentBlock.difficulty()));
